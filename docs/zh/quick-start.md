...
区块链配置是对要部署私链的基本信息进行配置，添加节点后区块链配置将不能进行编辑和删除操作。
...
| :---------------------------------------: | :-------------------------------: |
|                区块链名称                |  私链的唯一标识       |
|                签名算法                |  ECkey(Secp256k1) 或 SM2 |
|                  p2pVersion                  | 私链的P2P版本号 |

### 创世账户

创世账户中默认且必须包含一个黑洞账户（Blackhole），用户需添加至少一个创世账户以保证搭建的私链可进行交易。
...
> 注：voteCount 支持范围为 0 - 9223372036854775807  > 注：address 和 privateKey 可点击自动生成按钮生成
...
在节点详情页，用户可以点击编辑对未部署的节点进行编辑。节点部署后，将不能进行编辑操作。

### 删除节点

在节点详情页，用户可以点击删除对未部署的节点可以进行删除。节点部署后，将不能进行删除操作。  **当最后一个节点被删除时，意味着当前链被删除，系统将自动切换到首页面，待用户重新初始化链信息。**
...
TRON FACTORY只支持全量部署，用户可选择单个节点部署或勾选多个节点进行批量部署。  

点击部署后，填写本地编译后的java-tron部署包路径即可开始节点部署，具体java-tron编译生成部署包请查看[部署文档](https://tronprotocol.github.io/documentation-zh/developers/deployment/)。

部署开始后，所有的节点将会依次进行部署，部署结束后可点击日志查看相应的部署结果，部署成功后节点状态将由停止变成运行中。

### 节点启停

部署成功后，可以进行节点的停止和继续操作。

点击停止，节点停止产块。停止后的节点可选择继续或者重新部署，**点击继续，节点继续产块，历史产块信息依然保留；点击部署，节点重新部署，历史产块信息将清空。**

### 节点重置

节点停止后，用户可点击重置进行节点重置。**此时节点配置信息将重置为默认配置，节点的历史产块信息将清空，节点只能重新部署，不再支持启停操作。**
